    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            ...
            for pos in distribution:
                if distribution[pos] > maxProb:
                    maxProb = distribution[pos]
                    maxPos = pos
            distanceToMax = self.distancer.getDistance(pacmanPosition, maxPos)
            if distanceToMax < minDistance:
                minDistance = distanceToMax
                closestGhostPos = maxPos
        minDistance = float('inf')
        bestPos = None
        for newPos in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, newPos)
